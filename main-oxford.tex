...
\begin{itemize}
\item Now minimize Dirichlet's energy over the \textit{closed convex cone} defined by  
\[
K = \{ v \in H^1_0(\Omega) \mid v \geq 0 \text{~a.e.}\}% = H^1_0(\Omega) \cap H^1_+(\Omega).
\] 
%\item  \visible<2->{$K$ is no longer an affine set, but it is a \textit{closed convex cone.}
%:
% $\alpha > 0$, $u \in K$ implies $\alpha u \in K$ and $\alpha \in (0,1)$ and $u, v \in K$ implies $\alpha u + (1-\alpha) v \in K$
%}
\end{itemize}

\begin{center}\textit{
\visible<2->{ We can no longer reduce the variational inequality to a system of equations, }\visible<3->{but we can use the geometry of $K$ to define an iterative method in which we \textbf{solve} a sequence of (nonlinear) equations.}
}
\end{center}
\end{frame}
...
\item \visible<2->{Keep in mind: The obstacle problem can be viewed as a \textit{mixed complementarity problem}: Find $(u,\lambda) \in H^1_0(\Omega) \times H^{-1}(\Omega)$ s.t.
\[
...
\]}\vspace{-2ex}
\item \visible<3->{
The Lagrange multiplier $\lambda$ has low regularity. 
\item This is the source of \textit{mesh dependence}\footnote{\tiny The number of iterations needed to reach the same stopping tolerance increases with mesh refinements.}  for methods that tacitly require $\lambda \in L^2(\Omega)$.
}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item
Introducing \textbf{latent variables} $\psi, \widetilde{\psi} \in W$ (latent space) such that $u = \exp(\psi)$ and $\widetilde{\psi} = \ln w$ \visible<2->{transforms \eqref{eq:primal-entro-poisson} into a nonlinear saddle point problem in $(u,\psi)$:
...
\visible<4->{
\begin{itemize}
\item For now, $V_h \subset H^1(\Omega)$, $W_h \subset L^1(\Omega)$ denote finite dimensional approximation spaces with parameter $h > 0$.
\end{itemize}
}
\end{frame}
...
\label{eq:Biactivity_SmoothManufacturedSolution}
	...
	\end{cases}
\end{equation}
\item \visible<2->{$\lambda \equiv 0$:  this is a \textbf{biactive} solution, meaning
...
\only<3>{\text{ Solve }\quad \min_{u \in K} J(u) \quad \text{ by iteratively solving }  \quad \min_{u \in K} J(u) + \alpha^{-1} D(u,u_{\text{old}}) \quad \text{instead.}}
\]
\only<1,2>{
\begin{itemize}
...
%$
\item $J$: a smooth coercive functional
and $D : K \times K \to \overline{\mathbb R}$ is a \alert{Bregman distance.}
\end{itemize}
}
